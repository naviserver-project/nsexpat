...
|
\---------------------------------------------------------------------------*/
int TclExpatObjCmd _ANSI_ARGS_((ClientData dummy, Tcl_Interp * interp, int objc, Tcl_Obj * CONST objv[]));
static int TclExpatInstanceCmd _ANSI_ARGS_((ClientData dummy, Tcl_Interp * interp, int objc, struct Tcl_Obj * CONST objv[]));
...
static int TclExpatInitializeParser _ANSI_ARGS_((Tcl_Interp * interp, TclGenExpatInfo * expat, int resetOptions));
static void TclExpatFreeParser _ANSI_ARGS_((TclGenExpatInfo * expat));
static int TclExpatParse _ANSI_ARGS_((Tcl_Interp * interp,
                                      TclGenExpatInfo * expat, char *data, int len, TclExpat_InputType type));
static int TclExpatConfigure _ANSI_ARGS_((Tcl_Interp * interp, TclGenExpatInfo * expat, int objc, Tcl_Obj * CONST objv[]));
static int TclExpatCget _ANSI_ARGS_((Tcl_Interp * interp, TclGenExpatInfo * expat, int objc, Tcl_Obj * CONST objv[]));

static int TclExpatGet _ANSI_ARGS_((Tcl_Interp * interp, TclGenExpatInfo * expat, int objc, Tcl_Obj * CONST objv[]));
static void TclExpatDispatchPCDATA _ANSI_ARGS_((TclGenExpatInfo * expat));
static void TclGenExpatElementStartHandler _ANSI_ARGS_((void *userdata, const XML_Char * name, const XML_Char ** atts));
static void TclGenExpatElementEndHandler _ANSI_ARGS_((void *userData, const XML_Char * name));
static void TclGenExpatCharacterDataHandler _ANSI_ARGS_((void *userData, const XML_Char * s, int len));

static void TclGenExpatProcessingInstructionHandler _ANSI_ARGS_((void *userData, const XML_Char * target,
                                                                 const XML_Char * data));
static int TclGenExpatExternalEntityRefHandler _ANSI_ARGS_((XML_Parser parser, const XML_Char * openEntityNames,
                                                            const XML_Char * base, const XML_Char * systemId,
                                                            const XML_Char * publicId));
static void TclGenExpatDefaultHandler _ANSI_ARGS_((void *userData, const XML_Char * s, int len));
...
static int TclGenExpatUnknownEncodingHandler _ANSI_ARGS_((void *encodingHandlerData, const XML_Char * name,
                                                          XML_Encoding * info));

static void TclGenExpatStartNamespaceDeclHandler _ANSI_ARGS_((void *userdata,
                                                              const XML_Char * prefix, const XML_Char * uri));
static void TclGenExpatEndNamespaceDeclHandler _ANSI_ARGS_((void *userData, const XML_Char * prefix));


/* Following added by ericm@scriptics, 1999.6.25 */
/* Prototype definition for the TclExpat comment handler */
static void TclGenExpatCommentHandler _ANSI_ARGS_((void *userData, const XML_Char * data));
...
static void TclGenExpatElementDeclHandler _ANSI_ARGS_((void *userData, const XML_Char * name, XML_Content * model));
static void TclGenExpatAttlistDeclHandler _ANSI_ARGS_((void *userData,
                                                       const XML_Char * elname, const XML_Char * name,
                                                       const XML_Char * type, const XML_Char * dflt, int isrequired));
...
static TclHandlerSet *CreateTclHandlerSet(name)
char *name;
{
    TclHandlerSet *handlerSet;

    handlerSet = (TclHandlerSet *) ns_malloc(sizeof(TclHandlerSet));
    handlerSet->name = ns_strcopy(name);
...
CHandlerSet *CHandlerSetCreate(name)
char *name;
{
    CHandlerSet *handlerSet;

    handlerSet = (CHandlerSet *) ns_malloc(sizeof(CHandlerSet));
    handlerSet->name = ns_strcopy(name);
    handlerSet->ignoreWhiteCDATAs = 0;
    handlerSet->nextHandlerSet = NULL;
...
int TclExpatObjCmd(dummy, interp, objc, objv)
ClientData dummy;
Tcl_Interp *interp;
int objc;
Tcl_Obj *CONST objv[];
{
    ...
    if (!(genexpat = (TclGenExpatInfo *) ns_malloc(sizeof(TclGenExpatInfo)))) {
        ns_free((char *) genexpat);
    ...
    Tcl_CreateObjCommand(interp, Tcl_GetString(genexpat->name),
                         TclExpatInstanceCmd, (ClientData) genexpat, TclExpatDeleteCmd);
    /*
...
static Tcl_Obj *FindUniqueCmdName(interp)
Tcl_Interp *interp;
{
    Tcl_Obj *name;
    Tcl_CmdInfo info;
...
static int TclExpatInitializeParser(interp, expat, resetOptions)
Tcl_Interp *interp;
...
int resetOptions;
{
    ...
    if (expat->parser) {
        ...
        while (activeCHandlerSet) {
            if (activeCHandlerSet->resetProc) {
                activeCHandlerSet->resetProc(expat->interp, activeCHandlerSet->userData);
            }
            activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
        }
    } else {
        if (expat->ns_mode) {
            if (!(expat->parser = XML_ParserCreate_MM(NULL, NULL, &expat->nsSeparator))) {
                Tcl_SetResult(interp, "unable to create expat parserNs", NULL);
                return TCL_ERROR;
            }
        } else {
            if (!(expat->parser = XML_ParserCreate_MM(NULL, NULL, NULL))) {
    ...
    XML_SetCharacterDataHandler(expat->parser, (XML_CharacterDataHandler) TclGenExpatCharacterDataHandler);
    XML_SetProcessingInstructionHandler(expat->parser,
                                        (XML_ProcessingInstructionHandler) TclGenExpatProcessingInstructionHandler);
    XML_SetDefaultHandlerExpand(expat->parser, (XML_DefaultHandler) TclGenExpatDefaultHandler);

    XML_SetNotationDeclHandler(expat->parser, (XML_NotationDeclHandler) TclGenExpatNotationDeclHandler);
    XML_SetExternalEntityRefHandler(expat->parser, (XML_ExternalEntityRefHandler) TclGenExpatExternalEntityRefHandler);
    XML_SetUnknownEncodingHandler(expat->parser,
                                  (XML_UnknownEncodingHandler) TclGenExpatUnknownEncodingHandler, (void *) expat);
    ...
    XML_SetCdataSectionHandler(expat->parser, TclGenExpatStartCdataSectionHandler, TclGenExpatEndCdataSectionHandler);
    ...
    XML_SetDoctypeDeclHandler(expat->parser, TclGenExpatStartDoctypeDeclHandler, TclGenExpatEndDoctypeDeclHandler);

    XML_SetXmlDeclHandler(expat->parser, TclGenExpatXmlDeclHandler);

    XML_SetEntityDeclHandler(expat->parser, TclGenExpatEntityDeclHandler);
    if (expat->noexpand) {
        XML_SetDefaultHandlerExpand(expat->parser, NULL);
        XML_SetDefaultHandler(expat->parser, (XML_DefaultHandler) TclGenExpatDefaultHandler);
    } else {
...
static void TclExpatFreeParser(expat)
TclGenExpatInfo *expat;
{
    ExpatElemContent *eContent, *eContentSave;
...
static int TclExpatInstanceCmd(clientData, interp, objc, objv)
...
Tcl_Obj *CONST objv[];
{
    ...
    enum options {
        EXPAT_CONFIGURE, EXPAT_CGET, EXPAT_ns_free, EXPAT_GET,
        EXPAT_PARSE, EXPAT_PARSECHANNEL, EXPAT_PARSEFILE, EXPAT_RESET
    };


    if (objc < 2) {
        Tcl_SetResult(interp, "wrong # args: should be \"parserCmd method ?arg ...?\"", TCL_STATIC);
        return TCL_ERROR;
    }
    ...
    case EXPAT_CONFIGURE:
        ...
        if (objc < 3) {
            Tcl_SetResult(interp, "wrong # args: should be " "\"parserCmd configure <option> ?value ...?\"", TCL_STATIC);
            return TCL_ERROR;
        }
    ...
    case EXPAT_ns_free:
        ...
        if (expat->parsingState > 1) {
            Tcl_SetResult(interp, "parser freeing not allowed from within " "callback", TCL_STATIC);
            result = TCL_ERROR;
        } else {
    ...
    case EXPAT_RESET:
        ...
        if (expat->parsingState > 1) {
            Tcl_SetResult(interp, "parser reset not allowed from within " "callback", TCL_STATIC);
            result = TCL_ERROR;
        } else {
...
static int TclExpatParse(interp, expat, data, len, type)
Tcl_Interp *interp;
TclGenExpatInfo *expat;
char *data;
int len;
TclExpat_InputType type;
{
    ...
    if (!expat->parsingState) {
        ...
        while (activeCHandlerSet) {
            if (activeCHandlerSet->initParseProc) {
                activeCHandlerSet->initParseProc(expat->interp, activeCHandlerSet->userData);
            }
    ...
    case EXPAT_INPUT_STRING:
        ...
        result = XML_Parse(expat->parser, data, len, expat->final);
        expat->parsingState = 1;
        break;
    ...
    case EXPAT_INPUT_CHANNEL:
        ...
        if (strcmp(Tcl_DStringValue(&dStr), "binary") == 0)
            useBinary = 1;
        else
            useBinary = 0;
    ...
    case EXPAT_INPUT_FILENAME:
        ...
        if (fd < 0) {
            Tcl_ResetResult(interp);
            Tcl_AppendResult(interp, "error opening file \"", data, "\"", (char *) NULL);
            return TCL_ERROR;
        }
        parser = expat->parser;
        expat->parsingState = 2;
        for (;;) {
            ...
            if (nread < 0) {
                close(fd);
                Tcl_ResetResult(interp);
                Tcl_AppendResult(interp, "error reading from file \"", data, "\"", (char *) NULL);
                expat->parsingState = 1;
                return TCL_ERROR;
            }
    ...
    if (!result) {
        if (expat->status == ERROR_IN_EXTREFHANDLER) {
            ...
            Tcl_ResetResult(interp);
            sprintf(s, "%d", (int) XML_GetCurrentLineNumber(expat->parser));
            Tcl_AppendResult(interp, "error \"",
                             XML_ErrorString(XML_GetErrorCode(expat->parser)), "\" at line ", s, " character ", NULL);
            sprintf(s, "%d", (int) XML_GetCurrentColumnNumber(expat->parser));
...
static int TclExpatConfigure(interp, expat, objc, objv)
Tcl_Interp *interp;
TclGenExpatInfo *expat;
int objc;
Tcl_Obj *CONST objv[];
{
    ...
    if (expat->firstTclHandlerSet && (strcmp("default", expat->firstTclHandlerSet->name) == 0)) {
        activeTclHandlerSet = expat->firstTclHandlerSet;
    }
    while (objc > 1) {
        if (Tcl_GetIndexFromObj(interp, objPtr[0], switches, "switch", 0, &optionIndex) != TCL_OK) {
            return TCL_ERROR;
        }
        ...
        case EXPAT_IGNOREWHITECDATA:   /* -ignorewhitecdata */
            ...
            CheckDefaultTclHandlerSet;
            if (Tcl_GetBooleanFromObj(interp, objPtr[1], &activeTclHandlerSet->ignoreWhiteCDATAs)
                != TCL_OK) {
                return TCL_ERROR;
            }
        ...
        case EXPAT_PARAMENTITYPARSING: /* -paramentityparsing */
            /* ericm@scriptics */
            if (Tcl_GetIndexFromObj(interp, objPtr[1], paramEntityParsingValues, "value", 0, &value) != TCL_OK) {
                return TCL_ERROR;
            }
            switch ((enum paramEntityParsingValues) value) {
            case EXPAT_PARAMENTITYPARSINGALWAYS:
                XML_SetParamEntityParsing(expat->parser, XML_PARAM_ENTITY_PARSING_ALWAYS);
                expat->paramentityparsing = XML_PARAM_ENTITY_PARSING_ALWAYS;
                break;
            case EXPAT_PARAMENTITYPARSINGNEVER:
                XML_SetParamEntityParsing(expat->parser, XML_PARAM_ENTITY_PARSING_NEVER);
                expat->paramentityparsing = XML_PARAM_ENTITY_PARSING_NEVER;
                break;
            case EXPAT_PARAMENTITYPARSINGNOTSTANDALONE:
                XML_SetParamEntityParsing(expat->parser, XML_PARAM_ENTITY_PARSING_UNLESS_STANDALONE);
                expat->paramentityparsing = XML_PARAM_ENTITY_PARSING_UNLESS_STANDALONE;
                break;
            }
            break;
...
static int TclExpatCget(interp, expat, objc, objv)
Tcl_Interp *interp;
TclGenExpatInfo *expat;
int objc;
Tcl_Obj *CONST objv[];
{
    ...
    if (Tcl_GetIndexFromObj(interp, objv[0], switches, "switch", 0, &optionIndex) != TCL_OK) {
        return TCL_ERROR;
    }
    activeTclHandlerSet = expat->firstTclHandlerSet;
    ...
    if (objc > 1) {
        if (objc != 3) {
            Tcl_WrongNumArgs(interp, 0, objv, "?-handlerset handlersetname? switch");
            return TCL_ERROR;
        }
        if ((enum switches) optionIndex != EXPAT_HANDLERSET) {
            Tcl_ResetResult(interp);
            Tcl_AppendResult(interp, "usage: parserObj cget ", NULL);
            Tcl_AppendResult(interp, "?-handlerset handlersetname? switch", NULL);
            return TCL_ERROR;
        }
        ...
        for (activeTclHandlerSet = expat->firstTclHandlerSet;
             activeTclHandlerSet != NULL; activeTclHandlerSet = activeTclHandlerSet->nextHandlerSet) {
        ...
        if (Tcl_GetIndexFromObj(interp, objPtr, switches, "switch", 0, &optionIndex) != TCL_OK) {
            return TCL_ERROR;
        }
    }
    ...
    case EXPAT_BASE:           /* -base */
        ...
        if (expat->finished) {
            Tcl_SetResult(interp, expat->baseURI != NULL ? Tcl_GetString(expat->baseURI) : "", NULL);
        } else {
    ...
    case EXPAT_PARAMENTITYPARSING:     /* -paramentityparsing */
        ...
        case XML_PARAM_ENTITY_PARSING_UNLESS_STANDALONE:
            ...
            break;
        default:
            Tcl_AppendResult(interp, "impossible '-paramentityparsing' return value!\n");
            return TCL_ERROR;
        }
        return TCL_OK;
 ...
 *----------------------------------------------------------------------------
 */
static int TclExpatGet(interp, expat, objc, objv)
Tcl_Interp *interp;
TclGenExpatInfo *expat;
int objc;
Tcl_Obj *CONST objv[];
{
    ...
    if (objc > 1) {
        Tcl_SetResult(interp, "Only one value may be requested at a time", TCL_STATIC);
        return TCL_ERROR;
    }

    if (Tcl_GetIndexFromObj(interp, objv[0], getSwitches, "switch", 0, &switchIndex) != TCL_OK) {
        return TCL_ERROR;
    }
...
static void TclExpatHandlerResult(expat, handlerSet, result)
TclGenExpatInfo *expat;
TclHandlerSet *handlerSet;
int result;
{
...
static void TclGenExpatElementStartHandler(userData, name, atts)
void *userData;
const char *name;
const char **atts;
{
    ...
    while (activeTclHandlerSet) {
        ...
        if (activeTclHandlerSet->elementstartObjProc != NULL) {
            ...
            result =
                activeTclHandlerSet->elementstartObjProc(activeTclHandlerSet->elementstartclientData, expat->interp, 3,
                                                         vector);
            ...
            if (activeTclHandlerSet->elementstartcommand != NULL) {
                ...
                Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) name, -1));
                 ...
                 */
                result = Tcl_EvalObjEx(expat->interp, cmdPtr, TCL_EVAL_GLOBAL | TCL_EVAL_DIRECT);
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->elementstartcommand) {
            activeCHandlerSet->elementstartcommand(activeCHandlerSet->userData, name, atts);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
...
static void TclGenExpatElementEndHandler(userData, name)
void *userData;
CONST char *name;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) userData;
    ...
    while (activeTclHandlerSet) {
        ...
        if (activeTclHandlerSet->elementendObjProc != NULL) {
            ...
            Tcl_Preserve((ClientData) expat->interp);
            result =
                activeTclHandlerSet->elementendObjProc(activeTclHandlerSet->elementendclientData, expat->interp, 2, vector);
            Tcl_Release((ClientData) expat->interp);
            TclExpatHandlerResult(expat, activeTclHandlerSet, result);
        } else {
            if (activeTclHandlerSet->elementendcommand != NULL) {
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->elementendcommand) {
            activeCHandlerSet->elementendcommand(activeCHandlerSet->userData, name);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
...
static void TclGenExpatStartNamespaceDeclHandler(userData, prefix, uri)
void *userData;
const char *prefix;
const char *uri;
{
    ...
    while (activeTclHandlerSet) {
        ...
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) uri, -1));
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->startnsdeclcommand) {
            activeCHandlerSet->startnsdeclcommand(activeCHandlerSet->userData, prefix, uri);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
...
static void TclGenExpatEndNamespaceDeclHandler(userData, prefix)
void *userData;
CONST char *prefix;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) userData;
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->endnsdeclcommand) {
            activeCHandlerSet->endnsdeclcommand(activeCHandlerSet->userData, prefix);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
...
static int TclExpatCheckWhiteData(pc, len)
char *pc;
int len;
{
    for (; len > 0; len--, pc++) {
...
static void TclGenExpatCharacterDataHandler(userData, s, len)
void *userData;
CONST char *s;
int len;
{
...
static void TclExpatDispatchPCDATA(expat)
TclGenExpatInfo *expat;
{
    ...
    if (expat->cdata == NULL || expat->status != TCL_OK) {
        return;
    }
    ...
    while (activeTclHandlerSet) {
        ...
        if (activeTclHandlerSet->datacommandObjProc != NULL) {
            ...
            Tcl_Preserve((ClientData) expat->interp);
            result = activeTclHandlerSet->datacommandObjProc(activeTclHandlerSet->datacommandclientData, expat->interp,
                                                             2, vector);
            Tcl_Release((ClientData) expat->interp);
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->datacommand) {
            ...
            if (!activeCHandlerSet->ignoreWhiteCDATAs || !onlyWhiteSpace) {
                activeCHandlerSet->datacommand(activeCHandlerSet->userData, s, len);
            }
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
...
static void TclGenExpatProcessingInstructionHandler(userData, target, data)
void *userData;
CONST char *target;
CONST char *data;
{
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->picommand) {
            activeCHandlerSet->picommand(activeCHandlerSet->userData, target, data);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    return;
...
static void TclGenExpatDefaultHandler(userData, s, len)
void *userData;
CONST char *s;
int len;
{
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->defaultcommand) {
            activeCHandlerSet->defaultcommand(activeCHandlerSet->userData, s, len);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->entityDeclCommand) {
            activeCHandlerSet->entityDeclCommand(activeCHandlerSet->userData,
                                                 ...
                                                 length, base, systemId, publicId, notationName);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    return;
...
static void TclGenExpatNotationDeclHandler(userData, notationName, base, systemId, publicId)
void *userData;
CONST char *notationName;
CONST char *base;
CONST char *systemId;
CONST char *publicId;
{
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->notationcommand) {
            activeCHandlerSet->notationcommand(activeCHandlerSet->userData, notationName, base, systemId, publicId);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    return;
...
static int TclGenExpatUnknownEncodingHandler(encodingHandlerData, name, info)
...
XML_Encoding *info;
{
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->unknownencodingcommand) {
            activeCHandlerSet->unknownencodingcommand(activeCHandlerSet->userData, name, info);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    return 1;
 ...
 *----------------------------------------------------------------------------
 */
static int TclGenExpatExternalEntityRefHandler(parser, openEntityNames, base, systemId, publicId)
XML_Parser parser;
...
CONST char *publicId;
{
    ...
    while (activeTclHandlerSet) {
        ...
        if (base) {
            ...
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj("", 0));
        }
        ...
        case TCL_CONTINUE:
            ...
            break;
        case TCL_ERROR:
            TclExpatHandlerResult(expat, activeTclHandlerSet, ERROR_IN_EXTREFHANDLER);
            return 0;
        default:
            TclExpatHandlerResult(expat, activeTclHandlerSet, result);
            return 0;
        ...
        if (!extparser) {
            ...
            Tcl_SetResult(expat->interp, "unable to create expat external entity parser", NULL);
            TclExpatHandlerResult(expat, activeTclHandlerSet, ERROR_IN_EXTREFHANDLER);
            return 0;
        }
        ...
        while (activeCHandlerSet) {
            if (activeCHandlerSet->parserResetProc) {
                activeCHandlerSet->parserResetProc(extparser, activeCHandlerSet->userData);
            }
            activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
        }
        ...
        case EXPAT_INPUT_CHANNEL:
            ...
            if (!(mode & TCL_READABLE)) {
                ...
                Tcl_AppendResult(expat->interp, "channel \"", dataStr,
                                 "\" returned by the externalentitycommand ", "wasn't opened for reading", (char *) NULL);
                TclExpatHandlerResult(expat, activeTclHandlerSet, ERROR_IN_EXTREFHANDLER);
                Tcl_DecrRefCount(resultObj);
        ...
        case EXPAT_INPUT_FILENAME:
            ...
            if (fd < 0) {
                Tcl_ResetResult(expat->interp);
                Tcl_AppendResult(expat->interp, "error opening file \"", dataStr, "\"", (char *) NULL);
                TclExpatHandlerResult(expat, activeTclHandlerSet, ERROR_IN_EXTREFHANDLER);
                Tcl_DecrRefCount(resultObj);
                XML_ParserFree(extparser);
                expat->parser = oldparser;
            ...
            for (;;) {
                ...
                if (nread < 0) {
                    close(fd);
                    Tcl_ResetResult(expat->interp);
                    Tcl_AppendResult(expat->interp, "error reading from file \"", dataStr, "\"", (char *) NULL);
                    TclExpatHandlerResult(expat, activeTclHandlerSet, ERROR_IN_EXTREFHANDLER);
                    return 0;
                }
        ...
        if (!result) {
            Tcl_ResetResult(expat->interp);
            sprintf(s, "%d", (int) XML_GetCurrentLineNumber(extparser));
            ...
            sprintf(s, "%d", (int) XML_GetCurrentColumnNumber(extparser));
        ...
        while (activeCHandlerSet) {
            if (activeCHandlerSet->parserResetProc) {
                activeCHandlerSet->parserResetProc(oldparser, activeCHandlerSet->userData);
            }
            activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
        }
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->externalentitycommand) {
            if (activeCHandlerSet->externalentitycommand(activeCHandlerSet->userData, openEntityNames, base, systemId,
                                                         publicId)) {
                return 1;
            }
        }
 ...
 *----------------------------------------------------------------------------
 */
static void TclGenExpatCommentHandler(userData, data)
void *userData;
const char *data;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) userData;
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->commentCommand) {
            activeCHandlerSet->commentCommand(activeCHandlerSet->userData, data);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    return;
 ...
 *----------------------------------------------------------------------------
 */
static int TclGenExpatNotStandaloneHandler(userData)
void *userData;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) userData;
    Tcl_Obj *cmdPtr;
 ...
 *----------------------------------------------------------------------------
 */
static void TclGenExpatStartCdataSectionHandler(userData)
void *userData;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) userData;
    Tcl_Obj *cmdPtr;
 ...
 *----------------------------------------------------------------------------
 */
static void TclGenExpatEndCdataSectionHandler(userData)
void *userData;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) userData;
    Tcl_Obj *cmdPtr;
...
static void generateModel(interp, rep, model)
Tcl_Interp *interp;
Tcl_Obj *rep;
XML_Content *model;
{
...
static void TclGenExpatElementDeclHandler(userData, name, model)
void *userData;
const XML_Char *name;
XML_Content *model;
{
    ...
    eContent = (ExpatElemContent *) ns_malloc(sizeof(ExpatElemContent));
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->elementDeclCommand) {
            activeCHandlerSet->elementDeclCommand(activeCHandlerSet->userData, name, model);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
...
static void TclGenExpatAttlistDeclHandler(userData, elname, name, type, dflt, isrequired)
void *userData;
...
int isrequired;
{
    ...
    while (activeTclHandlerSet) {
        ...
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) elname, strlen(elname)));
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) name, strlen(name)));
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) type, strlen(type)));
        if (!dflt) {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj("", 0));
        } else {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) dflt, strlen(dflt)));
        }
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewIntObj(isrequired));
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->attlistDeclCommand) {
            activeCHandlerSet->attlistDeclCommand(activeCHandlerSet->userData, elname, name, type, dflt, isrequired);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    return;
...
static void TclGenExpatStartDoctypeDeclHandler(userData, doctypeName, sysid, pubid, has_internal_subset)
void *userData;
...
int has_internal_subset;
{
    ...
    while (activeTclHandlerSet) {
        ...
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) doctypeName, strlen(doctypeName)));
        if (sysid != NULL) {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) sysid, strlen(sysid)));
        } else {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj("NULL", 4));
        }
        if (pubid != NULL) {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) pubid, strlen(sysid)));
        } else {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj("NULL", 4));
        }
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewIntObj(has_internal_subset));
...
static void TclGenExpatEndDoctypeDeclHandler(userData)
void *userData;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) userData;
    Tcl_Obj *cmdPtr;
...
static void TclGenExpatXmlDeclHandler(userData, version, encoding, standalone)
void *userData;
const char *version;
const char *encoding;
int standalone;
{
    ...
    while (activeTclHandlerSet) {
        ...
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) version, -1));
        Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj((char *) encoding, -1));
        if (standalone == -1) {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewStringObj("", 0));
        } else {
            Tcl_ListObjAppendElement(expat->interp, cmdPtr, Tcl_NewBooleanObj(standalone));
        }
    ...
    while (activeCHandlerSet) {
        if (activeCHandlerSet->xmlDeclCommand) {
            activeCHandlerSet->xmlDeclCommand(activeCHandlerSet->userData, version, encoding, standalone);
        }
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
    }
    return;
...
static void TclExpatDeleteCmd(clientData)
ClientData clientData;
{
    TclGenExpatInfo *expat = (TclGenExpatInfo *) clientData;
    ...
    activeTclHandlerSet = expat->firstTclHandlerSet;
    while (activeTclHandlerSet) {
        ns_free(activeTclHandlerSet->name);
    ...
    while (activeCHandlerSet) {
        ...
        ns_free(activeCHandlerSet->name);

        tmpCHandlerSet = activeCHandlerSet;
        activeCHandlerSet = activeCHandlerSet->nextHandlerSet;
        ns_free((char *) tmpCHandlerSet);
    }
...
int CheckExpatParserObj(interp, nameObj)
Tcl_Interp *interp;
...
int CHandlerSetInstall(interp, expatObj, handlerSet)
Tcl_Interp *interp;
...
int CHandlerSetRemove(interp, expatObj, handlerSetName)
Tcl_Interp *interp;
Tcl_Obj *CONST expatObj;
char *handlerSetName;
{
    ...
    while (activeCHandlerSet) {
        if (strcmp(activeCHandlerSet->name, handlerSetName) == 0) {
            ...
            if (parentHandlerSet) {
                parentHandlerSet->nextHandlerSet = activeCHandlerSet->nextHandlerSet;
            } else {
                expat->firstCHandlerSet = activeCHandlerSet->nextHandlerSet;
            }
            ns_free((char *) activeCHandlerSet);
            return 0;
        }
...
CHandlerSet *CHandlerSetGet(interp, expatObj, handlerSetName)
Tcl_Interp *interp;
Tcl_Obj *CONST expatObj;
char *handlerSetName;
{
...
void *CHandlerSetGetUserData(interp, expatObj, handlerSetName)
Tcl_Interp *interp;
Tcl_Obj *CONST expatObj;
char *handlerSetName;
{
...
TclGenExpatInfo *GetExpatInfo(interp, expatObj)
Tcl_Interp *interp;
Tcl_Obj *CONST expatObj;
{
    Tcl_CmdInfo info;
